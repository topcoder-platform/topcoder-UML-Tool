...
/**
 * <p>
 * Unit test cases for <code>Aggregation</code>.
 * </p>
 *
 * @author littlebull
 * @version 1.0
 */
public class AggregationUnitTests extends TestCase {
    ...
    public void testCtor_Failure_NullName() {
        try {
            new Aggregation(null, multiplicity);
    ...
    public static void main(String[] args) {
        // create the Aggregation instance
        Aggregation edgeEnding = UnitTestsHelper.createAggregation();
        edgeEnding.setBorder(new LineBorder(Color.red));

        // set up the JFrame instance and show it
        JFrame frame = new JFrame();
        frame.setTitle("[Demo]Aggregation");
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

        frame.add(new JLabel("  right click the diamond inside to show popup menu ;)"), "North");
