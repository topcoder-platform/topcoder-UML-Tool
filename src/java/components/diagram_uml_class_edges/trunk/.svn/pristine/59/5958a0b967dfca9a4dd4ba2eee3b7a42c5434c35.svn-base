 ...
 * information is retrieved from the <code>Dependency</code> class from the UML Model and from the
 * <code>GraphEdge</code> associated with it. No active edge endings are supported. The class uses a dashed line, an
 * open ending and the simple arrow ending.
 * </p>
 *
 * <p>
 ...
 * @version 1.0
 */
public class DependencyEdge extends BaseEdge {
     ...
     */
    public DependencyEdge(GraphEdge graphEdge) throws IllegalGraphElementException {
        super(graphEdge, new LineStyle(DEFAULT_DASH_LENGTH, DEFAULT_BLANK_LENGTH), null, new OpenArrow());
    }
    ...
    public void notifyGraphEdgeChange(String message) throws IllegalGraphElementException {
        ...
        try {
            //Retrieve the Dependency element
            Uml1SemanticModelBridge semanticModelBridge =
                (Uml1SemanticModelBridge) this.getGraphEdge().getSemanticModel();
            Dependency dependency = (Dependency) semanticModelBridge.getElement();

            //Update name, stereotype compartments' text
            getNameCompartment().setText(dependency.getName());
            getStereotypeCompartment().setText(Utils.retrieveStereotypeText(dependency.getStereotypes()));

        } catch (NullPointerException e) {
            throw new IllegalGraphElementException(getGraphEdge(), "expects a correct semantic model", e);
        } catch (ClassCastException e) {
            throw new IllegalGraphElementException(getGraphEdge(), "the element should be instance of Dependency", e);
        }
