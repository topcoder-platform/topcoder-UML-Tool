...
/**
 * <p>
 * Unit tests for <code>ZUMLCallActionImpl</code> class.
 * </p>
 *
 * @author zhengjuyu
 * @version 1.0
 */
public class ZUMLCallActionImplUnitTest extends TestCase {
    /**
    ...
    public static Test suite() {
        return new TestSuite(ZUMLCallActionImplUnitTest.class);
    }

    /**
     * <p>
     * Accuracy test for constructor <code>ZUMLCallActionImpl()</code>.
     * </p>
     *
     * <p>
    ...
    public void testCtor() {
        ZUMLActivityNode node = new ZUMLCallActionImpl();
        assertNotNull("Unable to create ZUMLCallActionImpl instance.", node);
    }
     ...
     * taggedValue with dataValue being "Ture" and type being CallAction TagDefinition.
     * And verify that simple attributes are copied to the converted instance.
     * </p>
     */
    public void testConvertToTCUML1() {
        AbstractZUMLActivityNode node = new ZUMLCallActionImpl();
        ...
        assertEquals("Its type's tagType is CallAction.", "CallAction",
            taggedValue.getType().getTagType());

        //Verify that the simple attributes are copied.
        assertEquals("The name should match.", "call", element.getName());
