...
/**
 * <p>
 * This class is a concrete <code>Node</code>. It takes its information from the FinalState from the UML Model and
 * from the <code>GraphNode</code> associated with it. It extends the <code>BaseNode</code> to share the common
...
public class FinalNode extends BaseNode {
    ...
    /**
     * <p>
     * Represents the margin ratio compared to the outer circle's diameter. Default set to 0.1.
     * </p>
     */
    private static final double MARGIN_RATIO = 0.1;
    ...
    /**
     * <p>
     * Creates a <code>FinalNode</code> instance.
     * </p>
     * @param graphNode
     ...
     */
    public FinalNode(GraphNode graphNode, Map<String, String> properties, Point position, Rectangle bound) {
        ...
        notifyGraphNodeChange("FinalNode instance created.");
    }
...
//        return Util.containedByCompartments(this, x, y);
    ...
    /**
     * <p>
     * Rendering process of this node. Draw an outer circle, then fill an inner circle.
     * </p>
     * <p>
...
//        graphics.drawOval((int) shape.getX(), (int) shape.getY(), (int) shape.getWidth(), (int) shape.getHeight());
        ...
        graphics.fillOval((int) Math.round(shape.getX() + MARGIN_RATIO * shape.getWidth()), (int) Math.round(shape
                .getY()
                + MARGIN_RATIO * shape.getHeight()), (int) Math.round(shape.getWidth() * (1 - 2 * MARGIN_RATIO)),
                (int) Math.round(shape.getHeight() * (1 - 2 * MARGIN_RATIO)));
