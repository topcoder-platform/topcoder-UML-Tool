...
public class XMIConvertersUtil {
     ...
     */
    private static final String ADDTIONALHANDLERS = "AddtionalHandlers";
    ...
    private static void configHandlers(ObjectFactory of, XMIReader reader,
        ...
        if (handlersProperty != null) {
            ...
            while (en.hasMoreElements()) {
                ...
                String key = keyProperty.getValue().trim();

                Object obj;

                //The value should be format like 'key' or 'key:identifier'
                if (key.indexOf(":") != -1) {
                    String identifier = key.substring(key.indexOf(":") + 1);
                    key = key.substring(0, key.indexOf(":"));

                    //':id' and 'key:' and 'key:id:xx" are all invaid format
                    if (key.equals("") || identifier.equals("") ||
                            (identifier.indexOf(":") != -1)) {
                        throw new XMIConverterConfigurationException(
                            "The key for AddtionalHandlers should be correct.");
                    }
        ...
        if (convertersProperty != null) {
            ...
            while (en.hasMoreElements()) {
                ...
                for (String key : keyProperty.getValues()) {
                    ...
                    //The key should be format like "key" or "key:identifier"
                    if (key.indexOf(":") != -1) {
                        String identifier = key.substring(key.indexOf(":") + 1);
                        key = key.substring(0, key.indexOf(":"));

                        //The format ":id", "key:" and "key:id:xxx" are invalid
                        if (key.equals("") || identifier.equals("") ||
                                (identifier.indexOf(":") != -1)) {
                            throw new XMIConverterConfigurationException(
                                "The converter's key is invalid.");
                        }
                    ...
                    if (handler == null) {
                        throw new XMIConverterConfigurationException(
                            "Could not assign a converter for a unexisted handler: " +
                            name.trim().replace('$', '.'));
                    }
     ...
     *
     * @throws IllegalArgumentException if any argument is null or any double argument is negative
     */
    public static void adjustGraphNodeSize(GraphNode graphNode,
        ...
        Helper.checkNegative(emptyWidth, "emptyWidth");
        Helper.checkNegative(emptyHeight, "emptyHeight");
        Helper.checkNegative(margin, "margin");
        ...
        for (DiagramElement element : diagram.getContaineds()) {
            if (element instanceof GraphNode) {
                ...
                if (model instanceof SimpleSemanticModelElement) {
                    ...
                    //The title node's typeInfo should be NameCompartment
                    if (NAME_COMPARTMENT.equals(typeInfo)) {
                        GraphNode titleNode = (GraphNode) element;
                        ...
                        if (model instanceof SimpleSemanticModelElement) {
                            ...
                            typeInfo = ((SimpleSemanticModelElement) model).getTypeInfo();
        ...
        for (DiagramElement element : diagram.getContaineds()) {
            if (element instanceof GraphNode) {
                ...
                if (model instanceof SimpleSemanticModelElement) {
                    ...
                    //Check the typeInfo
                    if (NAME_COMPARTMENT.equals(typeInfo)) {
                        diagram.removeContained(element);
