...
/**
 * Accuracy test cases for class <code>PointTransformer</code>.
 *
 * @author Chenhong
 * @version 1.0
 */
public class TestPointTransformerAccuracy extends TestCase {
	/**
     * Represents the DiagramInterchangeElementTransformer instance for testing.
     */
	private DiagramInterchangeElementTransformer transformer = new PointTransformer();
	...
	public void testTransform() throws Exception {
        ...
        assertEquals("Equal is expected.", node.getNodeName(), "UML:Point");
        assertEquals("Equal is expected.", node.getAttributes().getLength(), 2);
        assertEquals("x should be 10", node.getAttribute("x"), Double.toString(element.getX()));
        assertEquals("y should be 100", node.getAttribute("y"), Double.toString(element.getY()));
	}
}