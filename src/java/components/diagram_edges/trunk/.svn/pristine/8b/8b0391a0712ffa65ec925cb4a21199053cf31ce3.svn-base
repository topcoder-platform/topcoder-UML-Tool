...
public abstract class Edge extends JComponent {
	...
	public void notifyChangeWayPoint(int index, Point oldPoint) {
		...
		if (index - 1 >= 0) {
			...
			if (index == wayPoints.size() - 1) {
				rotateTextFields(at, rightAnchoredTextFields, line);
			}
		...
		if (index + 1 < wayPoints.size()) {
			...
			if (index == 0) {
				rotateTextFields(at, leftAnchoredTextFields, line);
			}
	...
	private void rotateTextFields(AffineTransform at,
			List<TextField> textFields, Line2D line) {
		for (TextField textField : textFields) {
			double dist = textField.getAnchorage().distance(
					textField.getLocation());

			Point newLoc = transform(at, textField.getLocation());
			textField.setLocation(newLoc);
			...
			double dist2 = textField.getAnchorage().distance(
					textField.getLocation());

			Double c = Math.abs(dist - (dist / dist2));
			if (!c.isNaN() && c > 0.02 && dist2 > 0.00001) {
				at = new AffineTransform();
				at.setToTranslation(textField.getAnchorage().x, textField
						.getAnchorage().y);
				double d = Math.abs(dist / dist2);
				at.scale(d, d);
				at.translate(-textField.getAnchorage().x, -textField
						.getAnchorage().y);
				textField.setLocation(transform(at, textField.getLocation()));
			}
