...
/**
 * <p>
 * This action is used to add a JoinNode to the diagram view. It will create model element and graph element, then add
 * them to uml model manager and diagram first. This action is an UndoableAction.
 * </p>
 * @author ly, FireIce, ylouis
 * @version 1.0
 */
public class AddJoinNodeAction extends AddNodeAction {
     ...
     * </p>
     */
    private static final String PRESENTATION_NAME = "Add JoinNode";
    ...
    /**
     * <p>
     * Creates an instance of <code>AddJoinNodeAction</code>.
     * </p>
     * @param mainFrame
     *            main frame window
     * @param view
     ...
     */
    public AddJoinNodeAction(MainFrame mainFrame, DiagramView view, Point position, Dimension size, String name) {
     ...
     */
    public AddJoinNodeAction(MainFrame mainFrame, DiagramView view, Point position, Dimension size, Pseudostate element) {
        super(PRESENTATION_NAME, mainFrame, view);
        this.position = position;
        this.size = size;
    ...
    protected void createModelElement() {
        if (element == null) {
            ...
            element.setKind(PseudostateKind.JOIN);
            element.setName(name);
            Uml1SemanticModelBridge bridge = (Uml1SemanticModelBridge) getDiagramView().getDiagram().getOwner();
            setAddModelElementAction(new com.topcoder.uml.actions.model.activity.AddJoinNodeAction(element,
    ...
    /**
     * <p>
     * Creates a new JoinNode.
     * </p>
     */
    protected void createNode() {
        JoinNode node = DeployHelper.createJoinNode(getMainFrame(), graphNode);
        setNode(node);
    }
}
