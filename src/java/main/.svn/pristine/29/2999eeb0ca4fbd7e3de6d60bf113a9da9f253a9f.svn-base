...
/**
 * <p>
 * This action is used to add an ActionState to the diagram view. It will create model element and graph element, then
 * add them to uml model manager and diagram first. This action is an UndoableAction.
 * </p>
 * @author ly, FireIce, ylouis
 * @version 1.0
 */
public class AddActionStateAction extends AddNodeAction {
     ...
     * </p>
     */
    private static final String PRESENTATION_NAME = "Add ActionState";
     ...
     * </p>
     */
    private com.topcoder.uml.model.activitygraphs.ActionState element;
    ...
    /**
     * <p>
     * Creates an instance of <code>AddActionStateAction</code>.
     * </p>
     * @param mainFrame
     *            main frame window
     * @param view
     ...
     */
    public AddActionStateAction(MainFrame mainFrame, DiagramView view, Point position, Dimension size, String name) {
     ...
     */
    public AddActionStateAction(MainFrame mainFrame, DiagramView view, Point position, Dimension size,
        com.topcoder.uml.model.activitygraphs.ActionState element) {
        super(PRESENTATION_NAME, mainFrame, view);
        this.position = position;
        this.size = size;
    ...
    protected void createModelElement() {
        if (element == null) {
            element = new ActionStateImpl();
            element.setName(name);
            Uml1SemanticModelBridge bridge = (Uml1SemanticModelBridge) getDiagramView().getDiagram().getOwner();
            setAddModelElementAction(new com.topcoder.uml.actions.model.activity.AddActionStateAction(element,
    ...
    /**
     * <p>
     * Creates a new ActionState node.
     * </p>
     */
    protected void createNode() {
        ActionState node = DeployHelper.createActionStateNode(getMainFrame(), graphNode);
        setNode(node);
    }
