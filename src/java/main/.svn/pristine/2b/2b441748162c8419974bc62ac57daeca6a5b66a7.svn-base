...
/**
 * <p>
 * This action is used to add a MergeNode to the diagram view. It will create model element and graph element, then add
 * them to uml model manager and diagram first. This action is an UndoableAction.
 * </p>
 * @author ly, FireIce, ylouis
 * @version 1.0
 */
public class AddMergeNodeAction extends AddNodeAction {
     ...
     * </p>
     */
    private static final String PRESENTATION_NAME = "Add MergeNode";
    ...
    /**
     * <p>
     * Creates an instance of <code>AddMergeNodeAction</code>.
     * </p>
     * @param mainFrame
     *            main frame window
     * @param view
     ...
     */
    public AddMergeNodeAction(MainFrame mainFrame, DiagramView view, Point position, Dimension size, String name) {
     ...
     */
    public AddMergeNodeAction(MainFrame mainFrame, DiagramView view, Point position, Dimension size, Pseudostate element) {
        super(PRESENTATION_NAME, mainFrame, view);
        this.position = position;
        this.size = size;
    ...
    protected void createModelElement() {
        if (element == null) {
            ...
            element.setKind(PseudostateKind.JUNCTION);
            element.setName(name);
            Uml1SemanticModelBridge bridge = (Uml1SemanticModelBridge) getDiagramView().getDiagram().getOwner();
            setAddModelElementAction(new com.topcoder.uml.actions.model.activity.AddMergeNodeAction(element,
    ...
    protected void createNode() {
        MergeNode node = DeployHelper.createMergeNode(getMainFrame(), graphNode);
        setNode(node);
    }
}
