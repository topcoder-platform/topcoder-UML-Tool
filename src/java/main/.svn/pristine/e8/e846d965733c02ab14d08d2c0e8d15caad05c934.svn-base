...
/**
 * <p>
 * This action is used to add a FlowFinalNode to the diagram view. It will create model element and graph element, then
 * add them to uml model manager and diagram first. This action is an UndoableAction.
 * </p>
 * @author ly, FireIce, ylouis
 * @version 1.0
 */
public class AddFlowFinalNodeAction extends AddNodeAction {
     ...
     * </p>
     */
    private static final String PRESENTATION_NAME = "Add FlowFinalNode";
    ...
    /**
     * <p>
     * Creates an instance of <code>AddFlowFinalNodeAction</code>.
     * </p>
     * @param mainFrame
     *            main frame window
     * @param view
     ...
     */
    public AddFlowFinalNodeAction(MainFrame mainFrame, DiagramView view, Point position, Dimension size, String name) {
     ...
     */
    public AddFlowFinalNodeAction(MainFrame mainFrame, DiagramView view, Point position, Dimension size,
        FinalState element) {
        super(PRESENTATION_NAME, mainFrame, view);
        this.position = position;
        this.size = size;
    ...
    protected void createModelElement() {
        if (element == null) {
            element = new FinalStateImpl();
            element.addTaggedValue(DeployHelper.createTagValue("FinalNodeType", "FlowFinalNode"));
            element.setName(name);
            Uml1SemanticModelBridge bridge = (Uml1SemanticModelBridge) getDiagramView().getDiagram().getOwner();
            setAddModelElementAction(new com.topcoder.uml.actions.model.activity.AddFlowFinalNodeAction(element,
    ...
    /**
     * <p>
     * Creates a new FlowFinalNode.
     * </p>
     */
    protected void createNode() {
        FlowFinalNode node = DeployHelper.createFlowFinalNode(getMainFrame(), graphNode);
        setNode(node);
    }
}
