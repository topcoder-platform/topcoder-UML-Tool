...
/**
 * <p>
 * This action is used to add an InitialNode to the diagram view. It will create model element and graph element, then
 * add them to uml model manager and diagram first. This action is an UndoableAction.
 * </p>
 * @author ly, FireIce, ylouis
 * @version 1.0
 */
public class AddInitialNodeAction extends AddNodeAction {
    ...
    /**
     * <p>
     * Creates an instance of <code>AddInitialNodeAction</code>.
     * </p>
     * @param mainFrame
     *            main frame window
     * @param view
     ...
     */
    public AddInitialNodeAction(MainFrame mainFrame, DiagramView view, Point position, Dimension size, String name) {
     ...
     */
    public AddInitialNodeAction(MainFrame mainFrame, DiagramView view, Point position, Dimension size,
        Pseudostate element) {
        super(PRESENTATION_NAME, mainFrame, view);
        this.position = position;
        this.size = size;
    ...
    protected void createModelElement() {
        if (element == null) {
            ...
            element.setKind(PseudostateKind.INITIAL);
            element.setName(name);
            Uml1SemanticModelBridge bridge = (Uml1SemanticModelBridge) getDiagramView().getDiagram().getOwner();
            setAddModelElementAction(new com.topcoder.uml.actions.model.activity.AddInitialNodeAction(element,
    ...
    /**
     * <p>
     * Creates a new InitialNode.
     * </p>
     */
    protected void createNode() {
        InitialNode node = DeployHelper.createInitialNode(getMainFrame(), graphNode);
        setNode(node);
    }
}
