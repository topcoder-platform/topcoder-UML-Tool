...
/**
 * <p>
 * This action is used to add a ForkNode to the diagram view. It will create model element and graph element, then add
 * them to uml model manager and diagram first. This action is an UndoableAction.
 * </p>
 * @author ly, FireIce, ylouis
 * @version 1.0
 */
public class AddForkNodeAction extends AddNodeAction {
     ...
     * </p>
     */
    private static final String PRESENTATION_NAME = "Add ForkNode";
    ...
    /**
     * <p>
     * Creates an instance of <code>AddForkNodeAction</code>.
     * </p>
     * @param mainFrame
     *            main frame window
     * @param view
     ...
     */
    public AddForkNodeAction(MainFrame mainFrame, DiagramView view, Point position, Dimension size, String name) {
     ...
     */
    public AddForkNodeAction(MainFrame mainFrame, DiagramView view, Point position, Dimension size, Pseudostate element) {
        super(PRESENTATION_NAME, mainFrame, view);
        this.position = position;
        this.size = size;
    ...
    protected void createModelElement() {
        if (element == null) {
            ...
            element.setKind(PseudostateKind.FORK);
            element.setName(name);
            Uml1SemanticModelBridge bridge = (Uml1SemanticModelBridge) getDiagramView().getDiagram().getOwner();
            setAddModelElementAction(new com.topcoder.uml.actions.model.activity.AddForkNodeAction(element,
    ...
    protected void createNode() {
        ForkNode node = DeployHelper.createForkNode(getMainFrame(), graphNode);
        setNode(node);
    }
}
